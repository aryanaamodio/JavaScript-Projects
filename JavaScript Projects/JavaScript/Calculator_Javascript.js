...
Display_Value:'0',
//This will hold the first operand for any expressions, we set ir to null for now.
First_Operand:null,
...
//This checks if the Wair_Second _Operand is true and sets Display_Value
...
function Input_Decimal(dot){
    //This ensures that accidental clicking of the decimal point does't
    // cause bugs in the operation.
    if(Calculator.Wait_Second_Operand===true)return;
    if(!Calculator.Display_Value.includes(dot)){
        //We are saying tha if the Display_Value doe not contain a decimal point
...
if(First_Operand==null){
    ...
    //If operator exist, propery lookup is performed for the operatro
    //in the Perform_Calculation object and the function tha matches the 
    //operator is exexcuted.
    let result=Perform_Calculation[operator](Value_Now,Value_of_Input);
    //Here we add a fixed amount of numbers after the decimal.
    result = Number(result).toFixed(9);
    //This will remove any trailg 0's
...
keys.addEventListener('click',(event)=>{
    ...
    //that was clicked.
    const {target}=event;
    //If the element that was clicke on is not a button, exit the function.
